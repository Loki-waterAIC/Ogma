        ...
        with open(json_path, "r") as file:
            ...
            if all([(i in data) for i in ["dotm_path", "macros", "singleFileMacro", "files"]]):
                ...
                runWordMacroWin.run_word_macro_on_files(
                    doc_paths=files, macro_names=macros, template_path=dotm_path, activeDocumentMacro=singleFileMacro
                )

    except FileNotFoundError:
        err_message: str = f"[ogma.run_json 0] File not found: {args.json}"
        if ogmaGlobal.VERBOSE_LEVEL:
            print(err_message)
        raise FileNotFoundError(err_message)
    except json.JSONDecodeError:
        err_message: str = f"[ogma.run_json 1] Error decoding JSON from file: {args.json}"
        if ogmaGlobal.VERBOSE_LEVEL:
            print(err_message)
