...
# make values
values: tuple[list[str], dict[str, str]] = OTV.modify_word_properties(file_paths=FILES)
...
json_path: str = os.path.join(abs_path, "tests")
json_path: str = os.path.join(json_path, "data.json")
with open(file=json_path, mode="w", encoding="utf-8") as f:
    json.dump(obj=data,fp=f)

# send json to ogma
subprocess.run(args=[OGMA_PYTHON_LOCATION, "-OO", OGMA_PYTHON_SCRIPT_LOCATION])
